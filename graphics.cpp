...
void fillScreenColor(Color color) {
	fillScreenRect(color, 0, 0, SCREEN_WIDTH, SCREEN_HEIGHT);
}
...
void printTextScreen(int x, int y, const char* text, u32 color)
{
	...
	for (c = 0; c < strlen(text); c++) {
		if (x < -8 || x + 8 > SCREEN_WIDTH || y < -8 || y + 8 > SCREEN_HEIGHT) break;
		char ch = text[c];
...
void printTextScreenVertLeft(int x, int y, const char* text, u32 color)
{
	int i, j;
	unsigned int c;
	u8 *font;
	...
	for (c = 0; c < strlen(text); c++) {
		...
		for (i = 0; i < 8; i++, font++) {
			...
			vram_ptr += 8-i;
			for (j = 0; j < 8; j++) {
				vram_tmp = vram_ptr + (j * PSP_LINE_SIZE);
				if ((*font & (128 >> j))) *vram_tmp = color;
			}
		}
		y += 8;
	}
...
void printTextScreenVertRight(int x, int y, const char* text, u32 color)
{
	int i, j;
	unsigned int c;
	u8 *font;
...
void printTextCenter(const char* text, u32 color)
{
	int x = (SCREEN_WIDTH / 2) - ((strlen(text) * 8) / 2);
	int y = (SCREEN_HEIGHT / 2) - 4;
	
	printTextScreen(x, y, text, color);
}

void printTextCenter(const char* text, Color color, int y)
{
	int x = (SCREEN_WIDTH / 2) - ((strlen(text) * 8) / 2);
...
void drawRectangleScreen(int x0, int y0, int w, int h, Color color)
{
	drawLineScreen(x0, y0, x0+w, y0, color); //top
	drawLineScreen(x0+w, y0, x0+w, y0+h, color); //right
	drawLineScreen(x0, y0+h, x0+w, y0+h, color); //bottom
	drawLineScreen(x0, y0, x0, y0+h, color); //left
}
